-- Fix trigger functions to properly handle JSONB to TEXT[] conversion
...
RETURNS TRIGGER AS $$
BEGIN
    -- Update resume_summaries table with proper JSONB to TEXT[] conversion
    UPDATE resume_summaries SET
        ...
        skills = CASE 
            WHEN NEW.parsed_data->'skills' IS NOT NULL AND jsonb_typeof(NEW.parsed_data->'skills') = 'array' THEN
                ARRAY(SELECT jsonb_array_elements_text(NEW.parsed_data->'skills'))
            ELSE ARRAY[]::TEXT[]
        END,
        languages = CASE 
            WHEN NEW.parsed_data->'languages' IS NOT NULL AND jsonb_typeof(NEW.parsed_data->'languages') = 'array' THEN
...
RETURNS TRIGGER AS $$
BEGIN
    -- Insert into resume_summaries table with proper JSONB to TEXT[] conversion

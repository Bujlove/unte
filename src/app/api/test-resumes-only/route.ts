...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Resumes-only test: ${file.name}, type: ${file.type}, size: ${file.size}`);
    ...
    const fileName = `resumes-only-${Date.now()}-${Math.random().toString(36).substring(2)}.${fileExt}`;
    ...
    return NextResponse.json({
      success: true,
      message: 'Resumes-only upload successful!',
      resumeId: resume.id,
      fileName: file.name,
    ...
    console.error('Resumes-only upload error:', error);
    return NextResponse.json({
      success: false,
